...
function DashboardPage() {
  ...
  const visible = scope ? students.filter((s) => scope.includes(s.department)) : students;

  const stats = useMemo(() => {
    const n = visible.length || 1;
    const avgAtt = visible.reduce((a, s) => a + attendancePct(s), 0) / n;
    const pending = visible.reduce((a, s) => a + feeDue(s), 0);
    const passPct = (visible.filter(isPass).length / n) * 100;
    return {
      total: visible.length,
      avgAtt: Math.round(avgAtt * 10) / 10,
      pending,
      passPct: Math.round(passPct * 10) / 10,
      lowAttendance: visible.filter((s) => attendancePct(s) < 75).length,
    };
  }, [visible]);
  ...
  const attendanceByDept = useMemo(
    () =>
      (scope ?? DEPARTMENTS).map((d) => {
        const list = visible.filter((s) => s.department === d);
  ...
  if (isStudent && me) {
    ...
    return (
      <AppShell
        ...
        subtitle={`${me.course} · Semester ${me.semester} · ${me.id}`}
      >
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard
            label="Attendance"
            value={`${attendancePct(me)}%`}
            icon={CalendarCheck}
            tone={attendancePct(me) < 75 ? "danger" : "success"}
          />
          <StatCard
            label="Fees Paid"
            value={inr(me.feePaid)}
            icon={IndianRupee}
            hint={`${inr(feeDue(me))} due`}
          />
          <StatCard
            label="Percentage"
            value={`${pct}%`}
            icon={TrendingUp}
            hint={`Grade ${gradeFor(pct)}`}
          />
          <StatCard
            label="Status"
            value={isPass(me) ? "Pass" : "Fail"}
            icon={Users}
            tone={isPass(me) ? "success" : "danger"}
          />
        </div>
        <Card>
          <CardHeader>
